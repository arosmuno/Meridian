...
- source_index: the NUMBER (1-30) of the EXACT headline from the HEADLINES list that this record is based on. This lets us attach the original article photo. If you cannot identify a single source headline, use 0.`;
...
export async function fetchDealsFromWeb() {
  ...
  if (heads && heads.length >= 3) {
    try {
      ...
      if (deals && deals.length) return attachMedia(deals, heads.slice(0, 30));
    } catch {
      /* cae al fallback */
    }
  }
