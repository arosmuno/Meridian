...
function getPrompt() {
  // Foco aleatorio en cada ciclo → mejor variedad y backfill más rápido
  const focus = FOCUS_AREAS[Math.floor(Math.random() * FOCUS_AREAS.length)];
  const today = new Date().toISOString().split('T')[0];

  return `Use Google Search to find 6 REAL financial news stories from the last 14 days. Focus: ${focus}
