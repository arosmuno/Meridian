...
- HARD TESTS. Apply these before writing anything:
    * "M&A" requires an ACTUAL TRANSACTION with a named acquirer AND a named target or asset. A company
      "expanding", "strengthening its presence", "winning an order", launching a product or being profiled
      is NOT M&A. If you cannot name both sides, do not output the record.
    * "Debt Advisory" requires a specific financing: a named borrower or issuer raising, refinancing or
      restructuring debt. Government bond auctions and central bank operations are NOT Debt Advisory.
    * "Macro" is restricted to interest rates, credit conditions, debt and financing markets, and the level
      of M&A or private equity activity. Housing affordability, retail opening hours, return-to-office
      policies, labour disputes and general social or political news are NOT Macro. Leave them out.
    * GEOGRAPHY: Europe only, with a preference for Spain and Portugal. Local news from the United States,
      Asia or Latin America must not be output unless a European company is a party to the transaction.
...
function normalize(arr) {
  ...
  // Code-level gate. The model mislabels aggressively, so scope is enforced here too.
  const blank = (s) => !s || /^n\/?a$/i.test(String(s).trim());
  const OUTSIDE_EUROPE = /(michigan|texas|florida|california|ohio|canada|brazil|india|china|japan|australia)/i;
  const inScope = (d) => {
    if (!IN_SCOPE.has(d.type)) return false;
    if (OUTSIDE_EUROPE.test(String(d.headline || '') + ' ' + String(d.geography || ''))) return false;
    // An M&A record without both sides named is not a transaction, it is a press release.
    if (d.type === 'M&A' && (blank(d.buyer) || blank(d.target))) return false;
    return true;
  };
  return arr.filter(inScope).map((d) => {
